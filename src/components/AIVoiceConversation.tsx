        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">AI Voice Conversation</h1>
                <p className="text-sm text-gray-600">Speak to your AI language partner</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto p-4 h-[calc(100vh-80px)] flex flex-col">
        <div className="flex-1 overflow-y-auto mb-6 space-y-4">
          ...
          {messages.map((message) => (
            ...
            >
              <div className={`p-2 rounded-full shadow-lg ${message.sender === 'ai' ? 'bg-gradient-to-r from-blue-500 to-blue-600' : 'bg-gradient-to-r from-green-500 to-green-600'}`}>
                {message.sender === 'ai' ? <Bot className="w-5 h-5 text-white" /> : <User className="w-5 h-5 text-white" />}
              </div>
              <div className={`max-w-xs lg:max-w-md px-5 py-4 rounded-3xl shadow-xl backdrop-blur-md bg-white/60 border-2 ${message.sender === 'ai' ? 'border-blue-300/60' : 'border-green-300/60'} ${message.sender === 'ai' ? 'hover:border-blue-500/80' : 'hover:border-green-500/80'} transition-all duration-300`}
                style={{ boxShadow: message.sender === 'ai' ? '0 4px 24px 0 rgba(59,130,246,0.08)' : '0 4px 24px 0 rgba(34,197,94,0.08)' }}
              >
                <p className="text-base leading-relaxed font-medium text-gray-800">{message.text}</p>
                <p className={`text-xs mt-2 ${message.sender === 'ai' ? 'text-blue-400' : 'text-green-400'} font-mono font-normal opacity-70`}>{message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
              </div>
            </motion.div>
          ))}
          </AnimatePresence>
